...
- 会话上下文模块 `Session`：一个类 `Session`，存储当前所需的所有上下文（用户登录栈，选择的 ISBN）。
- 命令执行模块 `Commands`：所有命令。基类为 `Command`，有虚函数 `execute`，在执行过程中可能抛出异常。
...
- `class Session`：维护当前系统的运行时上下文，包括已登录的用户栈和当前选中的图书。
    - 方法：
        - `login_push()` / `login_pop()`：用户登入/登出。
        - `set_selected_book()` / `get_selected_book()`：设置/获取当前上下文选中的图书 ID。
        - `get_privilege()`：获取当前活跃用户的权限等级。
...
- `namespace util`：通用工具函数集合。
    - 函数：`split()` (字符串分割), `toInt()` (字符串转整数), `toDecimal()` (字符串转浮点金额), `to_array()` (string 转
      array)。
