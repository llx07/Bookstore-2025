...
- `server.cpp` 会启动一个服务器，监听端口 10086，同时托管了前端和后端。

## 功能设计

- 底层数据存储模块：
    - `MemoryRiver`：一个可以存储任意 POD 类型在文件中的类。
...
- 命令执行模块 `Commands`：所有命令。基类为 `Command`，有虚函数 `execute`，接受用户 ID，当前选择书，和一个 `std::ostream` 作为参数。
    - `BookCommands`：关于书本命令的实现。
    - `UserCommands`：关于用户命令的实现。
    - `LogCommands`：关于日志命令的实现。
- 命令解析模块
    - 函数 `parseCommand`，解析一个 `std::vector<std::string>`，返回一个 `Command` 或抛出异常。
...
- 用户信息 (user_data): 存储 User 结构体，包含用户ID、密码、用户名、权限，以及登录次数，用一个 BlockList 实现。
- 书籍信息 (book_data): 存储 Book 结构体，包含 ISBN、书名、作者、关键词、单价、库存，用 MemoryRiver 实现。
    - 索引文件: 为快速查询，为每个数据建立了索引。每个索引都是一个独立的 BlockList 文件：
...
- `class BooksManager`：Singleton，管理图书信息。
    - 方法：
        - `getInstance()`：获取单例实例。
        - `getBooksWith[ISBN/name/author/keyword]()`：根据不同条件检索图书，返回 `std::vector<Book>`。
        - `createBook()`：仅基于 ISBN 创建新书条目。
        - `modifyBookData()`：修改图书信息（同时更新相关索引）。
        - `importBook()`：进货（增加库存）。
        - `buyBook()`：购买图书（减少库存）。
- `class UsersManager`：Singleton，管理用户信息。
    - 方法：
        - `getInstance()`：获取单例实例。
        - `addUser()`：注册/添加新用户。
        - `isPasswordCorrect()`：验证密码。
        - `modifyPassword()`：修改密码。
        - `getUserByUserid()`：获取用户信息。
        - `eraseUser()`：删除用户。
- `class LogManager`：Singleton，管理日志信息。
    - 方法：
        - `getInstance()`：获取单例实例。
        - `addFinanceLog()`：追加一笔财务记录。
        - `getFinanceLog()`：查询最后 `cnt` 笔交易的总收支。
        - `addOperationLog()`：追加一条操作日志，返回日志ID。
        - `markOperationSuccess()`：标记操作日志为成功状态。
...
- `class HandlerRegistry`：命令解析注册表，实现字符串命令到处理函数的映射。
    - 方法：
        - `get_handler()`：根据命令第一个单词获取对应的解析器函数，返回若干个解析器函数。
- 全局函数: `parse_command()`：解析输入的 Token 列表，返回具体的 `Command` 对象指针，或者返回空指针，代表解析失败。

### 具体命令

- `class Command`：所有指令的抽象基类。
    - 方法：`execute(User::USERID_T& user_id, int selected_id, std::ostream& os)`：纯虚函数，执行具体逻辑。
    - 异常：`ExecutionException`。

- 具体的命令类
    - `class SwitchUserCommand`：处理 `su` 指令，用户登录或切换。
    - `class LogoutCommand`：处理 `logout` 指令，登出。
    - `class RegisterCommand`：处理 `register` 指令，注册。
    - `class AddUserCommand`：处理 `useradd` 指令，创建用户。
    - `class DeleteUserCommand`：处理 `delete` 指令，删除用户。
    - `class ChangePasswordCommand`：处理 `passwd` 指令，修改密码。
    ...
    - `class ReportFinanceCommand`：处理 `report finance` 指令，生成财务报告。
    - `class ReportEmployeeCommand`：处理 `report employee` 指令，生成员工报告。
    - `class LogCommand`：处理 `log` 指令，输出操作日志。
